...
void Texture2D::Generate(unsigned int width, unsigned int height, int** data)
{
    this->Width = width;
    this->Height = height;

    // Create texture from int array
    glBindTexture(GL_TEXTURE_2D, this->ID);
    glTexImage2D(GL_TEXTURE_2D, 0, this->Internal_Format, width, height, 0, this->Image_Format, GL_UNSIGNED_INT, *data);
